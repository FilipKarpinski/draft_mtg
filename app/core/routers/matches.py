...
@router.post("/")
def create_match(match: MatchCreate, db: Session = Depends(get_db)) -> MatchSchema:
    db_match = Match(
...
@router.put("/{match_id}")
def update_match(match_id: int, match: MatchSchema, db: Session = Depends(get_db)) -> MatchSchema:
    db_match = db.query(Match).filter(Match.id == match_id).first()
    if db_match is None:
        raise HTTPException(status_code=404, detail="Match not found")
